...
###Flagging missing values in incomplete problems. Asking unnecessary questions to the user in complete problems.
...
def handle_missing_information(problem_type, session_problem, user_input, detect_type_failed=False):
    ...
    if detect_type_failed:
        ...
        st.session_state.history.append(("assistant", "⚠️ Optimization type could not be detected. Please provide more details about the problem."))
    ...
    if error_message:
        ...
        st.session_state.history.append(("assistant", "### Missing Information:\n" + "\n".join([f"- {q}" for q in questions])))
        return  # Wait for the user to provide input in the existing text box
    ...
    if problem_type == "linear":
        ...
        st.session_state.history.append(("assistant", "❌ Unsupported problem type."))
        return
    ...
    if err2:
        ...
        if problem_type == "linear":
            ...
            formatted = format_nlp_solution(opt_val, opt_vars, modified_problem.get("objective_type", "minimize"), modified_problem)
        elif problem_type == "stochastic_programming":
            formatted = humanize_response(f"Optimal Value: {opt_val}\nVariable Values: {opt_vars}", "stochastic programming")
        else:

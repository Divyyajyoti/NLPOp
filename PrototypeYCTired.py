...
###Not highlighting missing values. Working fine for full problems.
...
def detect_missing_values(combined_input):
    """
    Detects missing values or variables in the user input.

    Args:
        combined_input (str): The combined user input history.

    Returns:
        tuple: A dictionary indicating completeness and a string error message if any issues are found.
    """
    # Use the current problem type and session problem for context
    problem_type_description = st.session_state.problem_type.replace('_', ' ') if st.session_state.problem_type else "unknown optimization"
...
def generate_missing_values_prompt(combined_input):
    """
    Generates a natural language prompt for the user to provide missing values or clarifications.

    Args:
        combined_input (str): The combined user input history.

    Returns:
        str: A natural language prompt for the user.
    """
    # Detect missing values or variables
    missing_values = detect_missing_values(combined_input)

    # Generate a humanized response for the missing values
    if missing_values:
        prompt = humanize_response(
            f"The following details are missing or unclear: {', '.join(missing_values)}. "
            "Could you provide more information or clarify these points?"
        )
    else:
        prompt = "The optimization type could not be detected. Could you provide more details about the problem?"
    ...
    with st.spinner("Processing..."):
      if st.session_state.problem_type is None:
        ...
        if not any(opt_types.values()):
            # Combine all user inputs so far for context
            combined_input = " ".join(st.session_state.user_input_history)
            missing_values_prompt = generate_missing_values_prompt(combined_input)
            
            # Append the prompt to the user input history
            st.session_state.user_input_history.append(missing_values_prompt)
            st.session_state.history.append(("assistant", missing_values_prompt))

        elif opt_types.get("linear_programming", False):
                st.session_state.problem_type = "linear"
                ...
                if combinatorial_data["problem_type"] == "tsp":
                    ...
                    st.session_state.history.append(("user", user_input))
                    st.session_state.history.append(("assistant", f"❌ Could not identify combinatorial problem type.  Error: {combinatorial_data['error']}"))
...
##################################
            ...
            if st.session_state.problem_type == "linear":
                ...
                if error or modified_knapsack is None:
                    ...
                    st.session_state.history.append(("user", user_input))
                    st.session_state.history.append(("assistant", human_response))
                    item_names = modified_knapsack["data"].get("item_names", [f"Item {i+1}" for i in range(len(values))])
                    st.success(f"🎯 Optimal Value: {optimal_value}")
                    display_knapsack_solution(values, weights, item_names, chosen_items)

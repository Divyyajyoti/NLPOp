...
context_df = pd.read_excel (r"C:\Users\admin\Downloads\Market Sizing - OptimizerR.xlsx", sheet_name='Q1 Base - Modified')
...
def forecast_and_rank_districts(df, trade_cols, amt_cols, risk_cols, trade_cost_col="2024-Q1 TRADES", budget=100):
    districts = df["DISTRICT"].unique()
    forecasted_risk, forecasted_profitability = [], []

    def forecast_next(series):
        try:
            model = SARIMAX(series, order=(1, 1, 1), seasonal_order=(1, 1, 1, 1),
    ...
    for district in districts:
        row = df[df["DISTRICT"] == district].squeeze()

        risk_series = row[risk_cols]
        risk_series.index = pd.period_range("2023Q3", periods=3, freq='Q')
        risk_forecast = forecast_next(risk_series)

        trades = row[trade_cols]
        amts = row[amt_cols]
        profit_series = (trades + amts) / 2
        profit_series.index = pd.period_range("2023Q3", periods=3, freq='Q')
        profit_forecast = forecast_next(profit_series)

        forecasted_risk.append(risk_forecast)
        forecasted_profitability.append(profit_forecast)

    df["forecasted_risk"] = forecasted_risk
    df["forecasted_profitability"] = forecasted_profitability

    scaler = MinMaxScaler()
    df[["forecasted_risk", "forecasted_profitability"]] = scaler.fit_transform(df[["forecasted_risk", "forecasted_profitability"]])
    df["final_score"] = 0.5 * (1 - df["forecasted_risk"]) + 0.5 * df["forecasted_profitability"]

    df["cost"] = scaler.fit_transform(df[[trade_cost_col]]) * 100
    df["cost"] = df["cost"].round().astype(int)

    values = df["final_score"].tolist()
    weights = df["cost"].tolist()
    names = df["DISTRICT"].tolist()
    return solve_knapsack(values, weights, budget, names), df.sort_values(by="final_score", ascending=False)
# ...existing code...
...
if st.button("📊 Run SARIMA + Knapsack Ranking"):
    ...
    try:
        (opt_val, selected_districts), ranked_df = forecast_and_rank_districts(
            context_df.copy(), trade_cols, amt_cols, risk_cols, budget=100)

        st.success(f"🎯 Optimal Value Achieved: {opt_val:.3f}")
        st.write("📍 Top Selected Districts:")
        st.dataframe(ranked_df[ranked_df["DISTRICT"].isin(selected_districts)][["DISTRICT", "final_score", "cost"]])

        # Optional: Humanize the result
        explanation = f"Selected districts maximize profitability and minimize risk for the next quarter under a budget of 100 units."
        st.info(humanize_response(explanation, "SARIMA + knapsack selection"))
    except Exception as e:
            ...
            if not any(opt_types.values()):
                ...
                if error_message:
                    ...
                    if opt_types.get("linear_programming", False):
                        ...
                        # --- AUTO-TRIGGER SARIMA + KNAPSACK IF DISTRICT RANKING PROBLEM DETECTED ---
                        # Heuristic: If user_input mentions "district" and context_df has the right columns, run SARIMA+knapsack
                        ...
                        if (
                            ...
                            try:
                                ...
                                st.error(f"Error during forecasting and ranking: {e}")
                        # --- END SARIMA + KNAPSACK AUTO-TRIGGER ---
            ...
            if st.session_state.problem_type == "linear":
                ...
                if error_message:
                    ...
                    if opt_types.get("linear_programming", False):
                        ...
                        elif combinatorial_data["problem_type"] == "combinatorial knapsack":
                            # --- District-aware knapsack logic ---
                            if detect_district_problem_with_llm(user_input):
                                trade_cols = [
                                    "2023-Q3 TRADES", "2023-Q4 TRADES", "2024-Q1 TRADES",
                                ]
                                amt_cols = [
                                    "2023-Q3 SANCTIONED AMT", "2023-Q4 SANCTIONED AMT", "2024-Q1 SANCTIONED AMT",
                                ]
                                ...
                                try:
                                    values, weights, item_names, chosen_items, opt_val = forecast_and_prepare_knapsack(
                                        context_df.copy(), trade_cols, amt_cols, risk_cols, capacity=100
                                    )

                                    # Store fresh data inside combinatorial_data so the existing flow remains intact.
                                    combinatorial_data["data"] = {
                                        "values": values,
                                        "weights": weights,
                                        "capacity": 100,
                                    ...
                                    st.session_state.session_problem = combinatorial_data  # keep history
                                    st.success(f"🎯 Optimal District Portfolio Value: {opt_val:.3f}")
                                    display_knapsack_solution(values, weights, item_names, chosen_items)
                                    formatted_solution = f"Optimal Knapsack Value: {optimal_value}\nChosen Items: {chosen_items}"
                                    human_response = humanize_response(formatted_solution, "knapsack problem")

                                except Exception as err:
                                    st.error(f"District ranking failed: {err}")
                            else:
                                # Fallback to your original knapsack logic
                                values = combinatorial_data["data"].get("values", [])
                                weights = combinatorial_data["data"].get("weights", [])
                                capacity = combinatorial_data["data"].get("capacity")
                                #item_names = combinatorial_data["data"].get("item_names", [])
                                opt_val, chosen_items = solve_knapsack(values, weights, capacity, item_names)
                                st.success(f"Optimal Value: {opt_val}")
                                formatted_solution = f"Optimal Knapsack Value: {optimal_value}\nChosen Items: {chosen_items}"
                                human_response = humanize_response(formatted_solution, "knapsack problem")
                                st.session_state.session_problem = combinatorial_data
                                #st.session_state.history.append(
                        ...
                        if "error" in stochastic_data:
                            ...
                            if err2:
                                ...
                                for i in range(num_scenarios):
                                    ...
                                    st.session_state.history.append(("user", user_input))
                                    st.session_state.history.append(("assistant", human_response))
                                # ...existing code...                                for i in range(num_scenarios):

...
definitions = pd.read_excel (r"C:\Users\admin\Downloads\MarketSize.xlsx", sheet_name='Data definitions')
...
def forecast_and_solve_knapsack(df, capacity=5.0):
    ...
    metrics = {
        "2024-Q3 SANCTIONED AMT": ["2023-Q3 SANCTIONED AMT", "2023-Q4 SANCTIONED AMT", "2024-Q1 SANCTIONED AMT", "2024-Q2 SANCTIONED AMT"],
        "2024-Q3 TRADES": ["2023-Q3 TRADES", "2023-Q4 TRADES", "2024-Q1 TRADES", "2024-Q2 TRADES"],
        "% 90+  in 2024-Q3": ["% 90+  in 2023-Q3", "% 90+ in 2023-Q4", "% 90+  in 2024-Q1", "% 90+  in 2024-Q2"]
    }
    ...
    def forecast_series(series):
        try:
            ...
            if series.isna().any() or len(series.dropna()) < 3:
                return np.nan
            model = SARIMAX(series, order=(1,1,1), seasonal_order=(1,1,1,4), enforce_stationarity=True, enforce_invertibility=True)
            result = model.fit(disp=False)
            forecast = result.forecast(steps=1)[0]
            return forecast if not np.isnan(forecast) else np.mean(series)
        except Exception as e:
            logging.warning(f"SARIMA failed: {e}")
    ...
    for district in districts:
        ...
        for new_col, old_cols in metrics.items():
            try:
                ...
                logging.error(f"Forecasting {new_col} for {district} failed: {e}")
    ...
    # Step 3: Compute knapsack values and weights
    df_forecasted["knapsack_value"] = (df_forecasted["scaled_trades"] + df_forecasted["scaled_amt"]) / 2
    df_forecasted["knapsack_weight"] = df_forecasted["scaled_risk"]
    ...
    print (df_forecasted["knapsack_value"], df_forecasted["knapsack_weight"])

    # Step 4: Solve knapsack
    optimal_value, selected_districts = solve_knapsack_now(values, weights, capacity, item_names=names)

    # Step 5: Decision flag
...
context_df, optimal_value, selected_districts = forecast_and_solve_knapsack(st.session_state.market_data, capacity=5.0)
                    ...
                    if opt_types.get("linear_programming", False):
                        ...
                        if (
                            ...
                            try:
                                ...
                                df_ranked, opt_val, chosen = forecast_and_solve_knapsack(context_df, capacity=5.0)

                                print(f"Optimal Knapsack Value: {opt_val}")
                                print("Selected Districts:", chosen)

                                # See full decision table
                                print(df_ranked[["DISTRICT", "knapsack_value", "knapsack_weight", "selected"]])

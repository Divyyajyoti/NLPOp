...
context_df = pd.read_excel(r"C:\Users\dhrvt\Downloads\MarketSize.xlsx", sheet_name='Q1 Base - Modified')
definitions = pd.read_excel(r"C:\Users\dhrvt\Downloads\MarketSize.xlsx", sheet_name='Data definitions')


# st.markdown(context_df)
...
def detect_optimization_type(problem_statement):
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an expert in optimization theory."},
...
def humanize_response(technical_output, problem_type="optimization"):
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are a formal mathematical assistant."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=1000
        )
...
def extract_lpp_from_text(text):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are a world-class mathematical assistant."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=1500
    )
...
def extract_dynamic_programming_problem(problem_text):
    prompt = f"""
    You are an expert in dynamic programming.

    From the following natural language problem, extract:
    - `states`: a description or list of the states
    - `transitions`: how one state moves to another
    - `recurrence_relation`: recurrence in pseudo-code or Python lambda form
    - `base_cases`: initial state values
    - `goal`: what needs to be optimized (min/max)
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an expert in dynamic programming."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=1000
        )
...
def solve_dp(dp_dict):
    try:
        # You can parse `recurrence_relation` using `exec`/`eval` carefully if safe,
        # or implement common patterns manually
        if "knapsack" in dp_dict.get("description", "").lower():
            return solve_knapsack(dp_dict['values'], dp_dict['weights'], dp_dict['capacity'])
        else:
            return "Dynamic Programming support for this structure is not yet implemented.", None
    except Exception as e:
...
def ask_user_to_fill_missing_values(problem_dict):
    """
    Prompts the user to fill in missing values in the problem dictionary.
    Uses the model to generate questions and humanizes the response.
    """
    ...
    # Generate and display questions for missing fields
    for field in missing_fields:
        ...
        try:
            response = openai.chat.completion.create(
                ...
                messages=[
                    {"role": "system",
                     "content": "You are an assistant generating questions for missing optimization problem values."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=200
            )
            ...
            if user_input:
                # Update the problem dictionary
                if isinstance(problem_dict[field], list):
                    problem_dict[field] = eval(user_input)  # Convert string input to list
                else:
...
def display_constraints(lpp_dict):
    ...
    if lpp_dict.get('A_eq'):
        ...
        for i, (row, b) in enumerate(zip(lpp_dict['A_eq'], lpp_dict['b_eq'])):
            constraint = " + ".join(f"{round(coef, 2)}{varNames[j]}" for j, coef in enumerate(row))
            idx = start_idx + i
            name = con_names[idx] if idx < len(con_names) else f"Constraint {idx + 1}"
            details.append(f"{name}: {constraint} = {b}")
...
def modify_lpp(session_problem, user_input):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an assistant modifying a Linear Programming Problem."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=1000
    )
...
def generate_prompt(missing_type, variable_name=None, constraint_name=None):
    if missing_type == "A_eq":
        prompt = f"What is the coefficient of variable '{variable_name}' in constraint '{constraint_name}'?"
    elif missing_type == "b_eq":
        prompt = f"What is the right-hand side value for constraint '{constraint_name}'?"
    else:
        prompt = f"Please enter a value."

    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an assistant generating prompts for missing values."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=200
    )
...
def ask_user(session_problem, problem_type, user_input, model, count):
    prompt = f"""
    You are an expert in optimisation theory, understanding the type of problems, the values needed to solve them, and flagging incomplete values.
    You also understand the different ways in which a problem can be understood and solved.
    You are very helpful also and understand the different types of questions you can ask the user to help them reach a valuable solution from the given problem statements.
    These questions can range from asking the missing values of variables to asking them about the type of optimisation techniques to be used by giving them a list of techniques which can feasibly and accurately solve the questions.
    Consider the following data:
    - Currently on-going problem in specific format: {session_problem}
    - User input at this time: {user_input}
    - Type of problem as recognised by the LLM using the user input: {problem_type}

    Read and understand all these values. Now frame a list of questions as you find relevant. These questions should ask the user about:

    - Missing values, if any, that are mandatorily required to solve the question using at least one type of logic.
    - Type of optimisation technique that the user prefers using by giving them a list of techniques that are applicable to this question.
    - Any further concerns the user may have with the problem statement or solution.
    - Any other questions that seem relevant to you based on the problem definition and user requirements.

    Return this in the form of a python list which contains strings of the questions to be asked to the user. Be professional and instructive.
    """
    ...
    try:
        ...
        match = re.search(r'\[.*\]', response.choices[0].message.content, re.DOTALL)
        if match:
            question_list_str = match.group()
            questions = ast.literal_eval(question_list_str)
            for question in questions:
                st.markdown(question)
            return questions, None
        else:
            return None, "Invalid NLP format returned: List not found."
    except Exception as e:
...
def extract_nlp_components(problem_statement):
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an expert in extracting NLP components."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=1500
        )
...
def format_nlp_solution(opt_val, opt_vars, objective, nlp_dict):
    ...
    summary = f"**Objective Function:** {objective_str} ({objective})\n\n"  # Add first snippet here for Forecast+Knapsack routine
...
def modify_nlp(session_nlp, user_input):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an assistant modifying a Non-Linear Programming problem."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=1000
    )
...
def modify_scp(problem_type, user_input):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an assistant modifying a set covering optimization problem."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=1000
    )
...
def display_knapsack_solution(values, weights, item_names, chosen_items):
    ...
    for name, val, wt in zip(item_names, values, weights):
        selected = "✅ Yes" if name in chosen_items else "❌ No"
...
def solve_knapsack_now(values, weights, capacity, item_names=None):
    n = len(values)
    values = [int(v * 1000) for v in values]  # scale up for integer dp
    ...
    selected_names = [item_names[i] for i in selected] if item_names else selected
    optimal_value = dp[n][capacity] / 1000  # rescale back

    return optimal_value, selected_names


def forecast_and_solve_knapsack(df, capacity=5.0):
    import streamlit as st
    from statsmodels.tsa.statespace.sarimax import SARIMAX
    from sklearn.preprocessing import MinMaxScaler
    import numpy as np
    ...
    metrics = {
        "2024-Q3 SANCTIONED AMT": [
            "2023-Q3 SANCTIONED AMT", "2023-Q4 SANCTIONED AMT",
            "2024-Q1 SANCTIONED AMT", "2024-Q2 SANCTIONED AMT"
        ],
        "2024-Q3 TRADES": [
            "2023-Q3 TRADES", "2023-Q4 TRADES",
            "2024-Q1 TRADES", "2024-Q2 TRADES"
        ],
        "% 90+  in 2024-Q3": [
            "% 90+  in 2023-Q3", "% 90+ in 2023-Q4",
            "% 90+  in 2024-Q1", "% 90+  in 2024-Q2"
        ]
    }
    ...
    def forecast_series(series):
        try:
            ...
            if series.isna().any() or len(series.dropna()) < 3:
                return np.nan
            model = SARIMAX(series, order=(1, 1, 1), seasonal_order=(1, 1, 1, 4),
                            enforce_stationarity=True, enforce_invertibility=True)
            result = model.fit(disp=False)
            forecast = result.forecast(steps=1)[0]
            return forecast if not np.isnan(forecast) else np.mean(series)
        except Exception as e:
            st.warning(f"SARIMA failed: {e}")
            return np.mean(series)

    for district in districts:
        row = df[df["DISTRICT"] == district].copy().squeeze()
        new_row = row.copy()

        for new_col, old_cols in metrics.items():
            try:
                series = row[old_cols]
                series.index = pd.period_range(start="2023Q3", periods=4, freq='Q')
                forecast = forecast_series(series)
                new_row[new_col] = forecast
            except Exception as e:
                st.error(f"Forecasting {new_col} for {district} failed: {e}")
                new_row[new_col] = np.nan

        results.append(new_row)

    df_forecasted = pd.DataFrame(results)

    # Scaling
    scaler = MinMaxScaler()
    df_forecasted["scaled_trades"] = scaler.fit_transform(df_forecasted[["2024-Q3 TRADES"]])
    df_forecasted["scaled_amt"] = scaler.fit_transform(df_forecasted[["2024-Q3 SANCTIONED AMT"]])
    df_forecasted["scaled_risk"] = scaler.fit_transform(df_forecasted[["% 90+  in 2024-Q3"]])

    w_trades = 0.5
    w_amt = 0.5
    w_risk = 1.0
    df_forecasted["knapsack_value"] = w_trades * df_forecasted["scaled_trades"] + w_amt * df_forecasted["scaled_amt"]
    df_forecasted["knapsack_weight"] = w_risk * df_forecasted["scaled_risk"]

    values = df_forecasted["knapsack_value"].tolist()
    weights = df_forecasted["knapsack_weight"].tolist()
    names = df_forecasted["DISTRICT"].tolist()

    # Solve knapsack
    optimal_value, selected_districts = solve_knapsack_now(values, weights, capacity, item_names=names)
    df_forecasted["selected"] = df_forecasted["DISTRICT"].isin(selected_districts)

    # Streamlit display
    st.subheader("📊 Forecasted Demand & Risk (2024-Q3)")
    st.dataframe(df_forecasted[[
        "DISTRICT", "2024-Q3 TRADES", "2024-Q3 SANCTIONED AMT", "% 90+  in 2024-Q3",
        "scaled_trades", "scaled_amt", "scaled_risk", "knapsack_value", "knapsack_weight"
    ]])

    # Rank selected locations
    st.subheader("🏆 Recommended Locations to Open New Branch")
    selected_df = df_forecasted[df_forecasted["selected"] == True].copy()
    selected_df["score"] = selected_df["knapsack_value"] / (selected_df["knapsack_weight"] + 0.01)
    selected_df.sort_values("score", ascending=False, inplace=True)

    st.write("Top-ranked districts based on high consistent demand and low risk:")
    st.dataframe(selected_df[[
        "DISTRICT", "2024-Q3 TRADES", "2024-Q3 SANCTIONED AMT", "% 90+  in 2024-Q3", "score"
    ]])

    # Plotting stacked bar chart for top 10 districts
    st.subheader("📊 Forecasted Metrics for Top 10 Districts")
    top_5_df = selected_df.head(10)  # Get top 10 districts by score
    if not top_5_df.empty:
        # Create a stacked bar chart using Plotly
        fig = go.Figure(data=[
            go.Bar(
                name="2024-Q3 TRADES",
                y=top_5_df["DISTRICT"],  # Districts on y-axis
                x=top_5_df["2024-Q3 TRADES"],  # Values on x-axis
                orientation='h',  # Horizontal bars
                marker_color='rgb(55, 83, 109)'  # Blue
            ),
            go.Bar(
                name="2024-Q3 SANCTIONED AMT",
                y=top_5_df["DISTRICT"],
                x=top_5_df["2024-Q3 SANCTIONED AMT"],
                orientation='h',
            ...
            go.Bar(
                name="% 90+ in 2024-Q3",
                y=top_5_df["DISTRICT"],
                x=top_5_df["% 90+  in 2024-Q3"],
                orientation='h',
                marker_color='rgb(255, 99, 71)'  # Tomato Red
            )
        ])

        # Update layout for better readability
        fig.update_layout(
            title="Forecasted Metrics for Top 10 Districts (2024-Q3)",
            xaxis_title="Value",
        ...
        st.write("No districts were selected for visualization due to insufficient data.")

    # Enhanced insights for top 10 districts
    st.subheader("🔍 Insights on Top 10 Districts")
    top_10_df = selected_df.head(10)  # Get top 10 districts by score
    if not top_10_df.empty:
        # Calculate averages for the top 10 districts
        avg_trades = top_10_df["2024-Q3 TRADES"].mean()
        avg_amt = top_10_df["2024-Q3 SANCTIONED AMT"].mean()
        avg_risk = top_10_df["% 90+  in 2024-Q3"].mean()

        # Get the top district's details
        top_district = top_10_df.iloc[0]["DISTRICT"]
        top_trades = top_10_df.iloc[0]["2024-Q3 TRADES"]
        top_amt = top_10_df.iloc[0]["2024-Q3 SANCTIONED AMT"]
        top_risk = top_10_df.iloc[0]["% 90+  in 2024-Q3"]
        top_score = top_10_df.iloc[0]["score"]

        # Calculate demand (TRADES + SANCTIONED AMT) for risk-reward analysis
        top_demand = top_trades + top_amt
        avg_demand = avg_trades + avg_amt

        # Analyze trends: Check if demand is increasing across quarters for the top district
        top_district_row = df[df["DISTRICT"] == top_district].squeeze()
        trades_trend = [top_district_row[f"2023-Q{i} TRADES"] for i in range(3, 5)] + \
                       [top_district_row[f"2024-Q{i} TRADES"] for i in range(1, 3)]
        trades_trend = pd.Series(trades_trend).pct_change().mean() * 100  # Average % change
        amt_trend = [top_district_row[f"2023-Q{i} SANCTIONED AMT"] for i in range(3, 5)] + \
                    [top_district_row[f"2024-Q{i} SANCTIONED AMT"] for i in range(1, 3)]
        amt_trend = pd.Series(amt_trend).pct_change().mean() * 100  # Average % change

        # Generate insights in bullet points
        insights = [
            f"**Top District ({top_district}) Leadership**: {top_district} leads with a score of {top_score:.3f}, driven by a strong forecasted demand of {top_trades:.2f} trades and ₹{top_amt:.2f} lakhs in sanctioned amount for 2024-Q3.",
            f"**Comparison to Averages**: Compared to the top 10 districts' averages (TRADES: {avg_trades:.2f}, SANCTIONED AMT: ₹{avg_amt:.2f} lakhs), {top_district} exceeds expectations by {(top_trades / avg_trades - 1) * 100:.1f}% in trades and {(top_amt / avg_amt - 1) * 100:.1f}% in sanctioned amount, showcasing its high potential for branch expansion.",
            f"**Risk vs. Reward Balance**: With a delinquency rate of {top_risk:.2f}% (vs. the top 10 average of {avg_risk:.2f}%), {top_district} offers a strong risk-reward balance. Its combined demand ({top_demand:.2f}) is {(top_demand / avg_demand - 1) * 100:.1f}% above the average ({avg_demand:.2f}), while maintaining a manageable risk profile.",
            f"**Demand Trends in {top_district}**: Historical data shows a consistent growth trend in {top_district}, with trades increasing by an average of {trades_trend:.1f}% per quarter and sanctioned amounts growing by {amt_trend:.1f}% per quarter over the past year, indicating sustained demand.",
            f"**Top 10 Districts Overview**: Across the top 10 districts, the average risk remains low at {avg_risk:.2f}%, while demand varies significantly—ranging from {top_10_df['2024-Q3 TRADES'].min():.2f} to {top_10_df['2024-Q3 TRADES'].max():.2f} trades—suggesting diverse opportunities for targeted expansion.",
            f"**Growth Potential**: Districts like {top_district} with high demand and low risk are prime candidates for expansion, as they are likely to attract more customers while minimizing financial risks in 2024-Q3."
        ]

        # Display insights as bullet points
        st.markdown("\n".join([f"- {insight}" for insight in insights]))
    else:
        st.write("No districts were selected for insights due to insufficient data.")

    st.success(f"✅ Optimal Knapsack Value: {optimal_value:.3f}")
    st.download_button(
        label="📥 Download Ranked Locations",
        data=selected_df.to_csv(index=False),
        file_name="recommended_locations.csv",
        mime="text/csv"
    )

    return df_forecasted, optimal_value, selected_districts
...
def modify_combinatorial(session_com, user_input):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an assistant modifying a combinatorial optimization problem."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=1000
    )
...
def extract_stochastic_components(problem_text):
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an expert in stochastic optimization."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=1500
        )
...
def classify_user_input(user_input, session_problem=None):
    ...
    "followup" → if it's related to the existing one
    """
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an intelligent assistant classifying user instructions."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=50
        )
...
def extract_growth_coefficients_with_llm(text):
    ...
    response = openai.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "You are an expert extracting compound growth coefficients."},
            {"role": "user", "content": prompt}
        ],
        max_tokens=500
    )
...
def check_problem_completeness(problem_type, session_problem, user_input):
    ...
    Here is the context information to be used to help complete any problem based on your understanding of the problem statement and dataframe column definitions:
    ...
    try:
        response = openai.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": f"You are an expert in {problem_type_description} optimization."},
                {"role": "user", "content": prompt}
            ],
            max_tokens=8000
        )
...
st.title("🔢 Linear Programming Optimizer (Chat Interface)")
...
if 'problem_type' not in st.session_state:
    st.session_state.problem_type = ''  # Initialize problem_type
if 'user_input_history' not in st.session_state:
    st.session_state.user_input_history = ''
if 'market_data' not in st.session_state:
    st.session_state.market_data = pd.DataFrame()  # Initialize market data

# --- 📥 Excel File Upload Section ---
st.subheader("📄 Optional: Upload an Excel File for Problem Detection")
uploaded_file = st.file_uploader("Upload an Excel file (.xlsx)", type=["xlsx"])

st.session_state.market_data = context_df
print("This is context", context_df)
print("This is market data", st.session_state.market_data)
        ...
        if opt_types.get("linear_programming", False):
            ...
            if error:
                ...
                if err2:
                    ...
                    st.session_state.history.append(
                        ("assistant", "📄 **Excel file loaded and processed as Linear Programming!**"))
            ...
            if error:
                ...
                if err2:
                    ...
                    st.session_state.history.append(
                        ("assistant", "📄 **Excel file loaded and processed as Nonlinear Programming!**"))
                    st.session_state.history.append(("assistant", human_response))
            ...
            if "error" in stochastic_dict:
                ...
                if err2:
                    ...
                    st.session_state.history.append(
                        ("assistant", "📄 **Excel file loaded and processed as Stochastic Programming!**"))
                    st.session_state.history.append(("assistant", human_response))
            ...
            if combinatorial_data["problem_type"] == "tsp":
                ...
                if isinstance(result, str):
                    ...
                    st.session_state.session_problem = combinatorial_data
                    st.session_state.history.append(
                        ("assistant", "📄 **Excel file loaded and processed as Assignment Problem!**"))
                    st.session_state.history.append(("assistant", human_response))
                ...
                st.session_state.session_problem = combinatorial_data
                st.session_state.history.append(
                    ("assistant", "📄 **Excel file loaded and processed as Knapsack Problem!**"))
                st.session_state.history.append(("assistant", human_response))
                ...
                if isinstance(result, str):
                    ...
                    st.session_state.session_problem = combinatorial_data
                    st.session_state.history.append(
                        ("assistant", "📄 **Excel file loaded and processed as Set Covering Problem!**"))
                    st.session_state.history.append(("assistant", human_response))
...
if user_input:
    ...
    # Check if the input contains the word "forecast"
    if any(word in user_input.lower() for word in ["district", "rank", "predict","branch"]):
        st.success("🔍 Processing your forecasting question...")
        if st.session_state.market_data.empty:
            st.error("❌ Market data is not loaded. Please upload an Excel file.")
            st.session_state.history.append(("assistant", "❌ Market data is not loaded. Please upload an Excel file."))
        else:
            df_forecasted, optimal_value, selected_districts = forecast_and_solve_knapsack(
                st.session_state.market_data,
                capacity=5.0
            )
            st.session_state.history.append(
                ("assistant", "🏆 Here are the top-ranked districts based on high loan demand and low risk for affordable home loans.")
            )
    else:
        # Handle as an optimization problem
            ...
            if st.session_state.problem_type == '':
                ...
                if not any(opt_types.values()):
                    ...
                    if error_message:
                        ...
                        st.success("✅ The problem statement is complete and ready to solve!")
                        if opt_types.get("linear_programming", False):
                            st.session_state.problem_type = "linear"
                ...
                # Handle follow-up optimization problems
                if st.session_state.problem_type == "linear":
                    ...
                    if error or modified_tsp is None:
                        ...
                        if isinstance(distance, str):
                            st.session_state.history.append(("assistant", f"❌ {distance}"))
                        else:
                            formatted = format_tsp_solution(distance, path)
                            human_response = humanize_response(formatted, "traveling salesman problem")
                    ...
                    modified_scp, error = modify_scp(st.session_state.session_problem, user_input)
                    if error or modified_scp is None:
                        st.session_state.history.append(("assistant", f"❌ {error}"))
                    else:
                        sets = modified_scp["data"].get("sets", [])
                        universe = modified_scp["data"].get("universe", [])
                        result = solve_set_covering(sets, universe)
                        if isinstance(result, str):
                            ...
                            formatted = format_set_covering_solution(cover_indices, cost, sets)
                            human_response = humanize_response(formatted, "set covering problem")
                            st.session_state.session_problem = modified_scp
                            st.session_state.history.append(("assistant", human_response))

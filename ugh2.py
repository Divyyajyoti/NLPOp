...
context_df = pd.read_excel (r"C:\Users\admin\Downloads\MarketSize.xlsx", sheet_name='Q1 Base - Modified')
...
# ...existing code...
def forecast_and_rank_districts(df, trade_cost_col="2024-Q1 TRADES", amt_cost_cols="2024-Q1 SANCTIONED AMT",
                                risk_cost_cols="% 90+ in 2024-Q1", budget=100):
    """
    Forecasts profitability and risk for each district using SARIMA and ranks them using multi-objective optimization.
    Selects top 10 districts based on a score balancing profitability (0.6) and risk (0.4).

    Parameters:
    - df: DataFrame containing market data with specified columns.
    - trade_cost_col: Column for trade cost (default: '2024-Q1 TRADES').
    - amt_cost_cols: Column for sanctioned amount (default: '2024-Q1 SANCTIONED AMT').
    - risk_cost_cols: Column for risk (default: '% 90+ in 2024-Q1').
    - budget: Budget constraint (not used in multi-objective optimization but kept for compatibility).

    Returns:
    - results_df: DataFrame with forecasted risk, profitability, scores, and selection status.
    ...
    try:
        ...
        risk_cols = ["% 90+  in 2024-Q2", "% 90+  in 2024-Q1", "% 90+ in 2023-Q4", "% 90+  in 2023-Q3"]
        trade_cols = ["2023-Q3 TRADES", "2023-Q4 TRADES", "2024-Q1 TRADES", "2024-Q2 TRADES"]
        amt_cols = ["2023-Q3 SANCTIONED AMT", "2023-Q4 SANCTIONED AMT", "2024-Q1 SANCTIONED AMT", "2024-Q2 SANCTIONED AMT"]

        # Validate columns
        required_cols = risk_cols + trade_cols + amt_cols + ["DISTRICT"]
        ...
        def forecast_next(series):
            """
            Forecasts the next value in a series using SARIMA with robust error handling.
            """
            try:
                ...
                # SARIMA with parameters matching original analyze_data_sarima
                model = SARIMAX(
                    series,
                    order=(1, 1, 1),
                ...
                logging.warning(f"SARIMA forecasting failed: {e}")
                return np.mean(series)

        for district in districts:
            district_data = df[df["DISTRICT"] == district].squeeze()

            # Forecast risk
            risk_series = pd.to_numeric(district_data[risk_cols], errors='coerce')
            logging.debug(f"Risk series for {district}: {risk_series.values}")
            if len(risk_series) == 4 and not risk_series.isna().any():
                risk_series.index = pd.period_range(start="2023Q3", periods=4, freq='Q')
                risk_forecast = forecast_next(risk_series)
            else:
                logging.error(f"Risk series length mismatch or NaN for {district}")
                risk_forecast = np.nan

            # Forecast profitability (average of trades and sanctioned amounts)
            trades = pd.to_numeric(district_data[trade_cols], errors='coerce')
            amts = pd.to_numeric(district_data[amt_cols], errors='coerce')
            if len(trades) == 4 and len(amts) == 4 and not trades.isna().any() and not amts.isna().any():
                profit_series = (trades + amts) / 2
                profit_series.index = pd.period_range(start="2023Q3", periods=4, freq='Q')
                profit_forecast = forecast_next(profit_series)
            else:
                logging.error(f"Profit series length mismatch or NaN for {district}")
                profit_forecast = np.nan

            # Normalize and compute score (0.6 profit, 0.4 risk)
            profit_norm = (profit_forecast - np.nanmin(amts + trades)) / (
                    np.nanmax(amts + trades) - np.nanmin(amts + trades) + 1e-10)
            risk_norm = (risk_forecast - np.nanmin(risk_series)) / (
                    np.nanmax(risk_series) - np.nanmin(risk_series) + 1e-10)
            score = 0.6 * profit_norm - 0.4 * risk_norm

            results.append({
                'id': str(uuid.uuid4()),
                'district': district,
                'forecasted_profit': float(profit_forecast),
                'forecasted_risk': float(risk_forecast),
                'score': float(score),
                'selected': False
            })
        ...
        # Handle NaN values
        if results_df["forecasted_risk"].isna().all() or results_df["forecasted_profitability"].isna().all():
            raise ValueError("All forecasts are NaN. Check data or SARIMA model stability.")
        results_df["forecasted_risk"] = results_df["forecasted_risk"].fillna(results_df["forecasted_risk"].mean())
        results_df["forecasted_profit"] = results_df["forecasted_profit"].fillna(results_df["forecasted_profit"].mean())
        results_df["score"] = results_df["score"].fillna(results_df["score"].mean())

        # Multi-objective optimization: Select top 10 districts by score
        results_df['selected'] = results_df['score'].rank(ascending=False) <= 10

        return results_df, None
    except Exception as e:
        return None, f"Error in forecast_and_rank_districts: {e}"
...
st.session_state.market_data, error = forecast_and_rank_districts(st.session_state.market_data)
context_df = st.session_state.market_data
print (error)
print ("This is 2nd context", context_df)
print ("This is 2nd market data", st.session_state.market_data)
...
user_input = st.chat_input("Enter a new LPP description or a follow-up modification...") #Button to trigger SARIMA


if user_input:
    st.session_state.problem_type = classify_user_input(st.session_state.user_input_history, user_input)
    with st.spinner("Processing..."):
        if st.session_state.problem_type == '':
            ...
            if not any(opt_types.values()):
                ...
                if error_message:
                    ...
                    if opt_types.get("linear_programming", False):
                        ...
                        district_cols = ["DISTRICT", "2023-Q3 TRADES", "2023-Q4 TRADES", "2024-Q1 TRADES",
                                         "2023-Q3 SANCTIONED AMT", "2023-Q4 SANCTIONED AMT", "2024-Q1 SANCTIONED AMT",
                                         "% 90+  in 2023-Q3", "% 90+ in 2023-Q4", "% 90+  in 2024-Q1"]
                        if (
                                any("district" in str(user_input).lower() for _ in range(1)) and
                                all(col in context_df.columns for col in district_cols)
                        ):
                            try:
                                results_df, error = forecast_and_rank_districts(context_df.copy())
                                if error:
                                    st.error(f"❌ Error during forecasting and ranking: {error}")
                                else:
                                    st.success("🎯 SARIMA multi-objective analysis completed!")
                                    st.write("📍 Top Selected Districts:")
                                    st.dataframe(results_df[results_df["selected"]][
                                                     ["district", "forecasted_profit", "forecasted_risk", "score"]])
                                    explanation = (
                                        f"The top 10 districts were selected to maximize profitability and minimize risk for the next quarter. "
                                        f"Profitability is based on forecasted trades and sanctioned amounts, while risk is based on forecasted delinquency rates. "
                                        f"The score combines 60% weight on profit and 40% weight on risk (lower is better)."
                                    )
                                    st.info(humanize_response(explanation))
                                    # Add to chat history
                                    st.session_state.history.append(("assistant", explanation))
                            except Exception as e:
                                st.error(f"❌ Error during forecasting and ranking: {e}")
                        # --- END SARIMA + MULTI-OBJECTIVE OPTIMIZATION AUTO-TRIGGER ---
            ...
            if st.session_state.problem_type == "linear":
                st.session_state.history.append(("user", user_input))
                user_input = inject_compound_growth_coefficients(st.session_state.user_input_history)
                ...
                if not sets or not universe:
                    ...
                    if isinstance(result, str):
                        ...
                        st.session_state.history.append(("user", user_input))
                        st.session_state.history.append(("assistant", human_response))
            elif st.session_state.problem_type == "unknown optimization" or "unknown":
                completeness_check, error_message = check_problem_completeness(
                    problem_type=st.session_state.problem_type,
                    session_problem=st.session_state.session_problem,
                    user_input=st.session_state.user_input_history
                )
                    ...
                    if opt_types.get("linear_programming", False):
                        ...
                        if error:
                            ...
                            if err2:
                                ...
                                formatted = format_solution(opt_val, opt_vars, lpp_data.get("objective"),
                                                            lpp_data)
                                human_response = humanize_response(formatted)
                                st.session_state.session_problem = lpp_data
                                st.session_state.history.append(("user", user_input))
                                st.session_state.history.append(("assistant", human_response))
                            true_types = [key.replace("_", " ").title() for key, val in opt_types.items() if
                                          val]
                            if true_types:
                                st.session_state.history.append(("assistant",
                                                                 f"📚 **Detected Optimization Types:** `{', '.join(true_types)}`"))
                            else:
                                st.session_state.history.append(
                                    ("assistant", "📚 **Detected Optimization Types:** `None detected`"))
                    elif opt_types.get("nonlinear_programming", False):
